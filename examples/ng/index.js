...
var json = { title: "Product Feedback Survey Example", showProgressBar: "top", pages: [
    {
        questions: [
            { type: "radiogroup", name: "price to competitors", title: "Compared to our competitors, do you feel the Product is",
                choices: ["Less expensive", "Priced about the same", "More expensive", "Not sure"]},
            { type: "comment", name: "suggestions", title:"What would make you more satisfied with the Product?", placeHolder: "www" }
         ]
    },
    {
        questions: [
...
//model.data = { suggestions: "test suggs", "price to competitors": "Priced about the same" };
model.onComplete.add(function (s) {
    console.log("\nThe results are:\n" + JSON.stringify(model.data));
});

var ngApp = angular.module("app", ["ngSurvey", "ngSanitize"]);
